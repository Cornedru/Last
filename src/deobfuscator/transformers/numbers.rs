...
impl<'a> VisitMut<'a> for NumbersVisitor<'a> {
    ...
    fn visit_expression(&mut self, expr: &mut Expression<'a>) {
        if let Expression::StaticMemberExpression(memb) = &expr {
            if let Expression::Identifier(ident) = &memb.object {
                if let Some(props) = self.objects.get(&ident.name.to_string()) {
                    if let Some(value) = props.get(&memb.property.name.to_string()) {
