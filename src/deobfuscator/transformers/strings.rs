            ...
            if let Expression::StaticMemberExpression(member) = &call_expr.callee {
                if let Expression::StringLiteral(lit) = &member.object {
                    if member.property.name == "split" {
                        if let Some(Argument::StringLiteral(lit_2)) = &call_expr.arguments.first() {
...
impl<'a> DecoderVisitor<'a> {
    ...
    fn extract_decoder(&mut self, assign_expr: &AssignmentExpression<'a>) {
        if let AssignmentTarget::AssignmentTargetIdentifier(ident) = &assign_expr.left {
            if let Expression::BinaryExpression(bin) = &assign_expr.right {
                if let (
...
impl<'a> VisitMut<'a> for DecoderVisitor<'a> {
    fn visit_computed_member_expression(&mut self, node: &mut ComputedMemberExpression<'a>) {
        ...
        {
            if matches!(call_expr.callee, Expression::Identifier(_))
                && ident.name.as_str() == "JSON"
                && self.sub == 0
                && self.main_script
            {
                if let Some(Argument::NumericLiteral(lit)) = &call_expr.arguments.first() {
    ...
    fn visit_call_expression(&mut self, call_expr: &mut CallExpression<'a>) {
        if let Expression::Identifier(ident) = &call_expr.callee {
            if ident.name == "Error" && self.sub == 0 && !self.main_script {
                if let Some(Argument::CallExpression(call_expr_2)) = &call_expr.arguments.first() {
                    if let Some(Argument::NumericLiteral(lit)) = &call_expr_2.arguments.first() {
                        self.sub = lit.value as usize;
                    }
                }
            }
    ...
    fn visit_function_body(&mut self, body: &mut FunctionBody<'a>) {
        if !body.is_empty() {
            if let Statement::ExpressionStatement(expr) = &body.statements[0] {
                if let Expression::AssignmentExpression(assign_expr) = &expr.expression {
    ...
    fn visit_return_statement(&mut self, node: &mut ReturnStatement<'a>) {
        if let Some(Expression::SequenceExpression(sequ_expr)) = &node.argument {
            if sequ_expr.expressions.len() == 3 {
...
impl<'a> VisitMut<'a> for Strings<'a> {
    ...
    fn visit_expression(&mut self, node: &mut Expression<'a>) {
        if let Expression::CallExpression(call_expr) = node {
            ...
            {
                if let Argument::NumericLiteral(lit) = &call_expr.arguments[0] {
                    if let Some(decoded_str) = self.decoder.decode_string(lit.value as usize) {
