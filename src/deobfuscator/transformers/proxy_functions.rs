                                            ...
                                            Expression::ComputedMemberExpression(member_expr) => {
                                                    ...
                                                    &member_expr.expression
                                                {
                                                        ...
                                                        proxies.get(str_lit.value.as_str())
                                                    {
                                                        ...
                                                        // TODO: cas non supporté sans unsafe, nécessite refactoring des lifetimes
                                                        return (None, None);
                                                    }
                                                }
                                                return (None, None);
    ...
    fn visit_expression(&mut self, node: &mut Expression<'a>) {
        if let Expression::AssignmentExpression(assign_expr) = node {
            ...
            if let AssignmentTarget::ComputedMemberExpression(member_expr) = &assign_expr.left {
                if let Expression::StringLiteral(str_lit) = &member_expr.expression {
                    if !KEYS_BLACKLIST.contains(&str_lit.value.as_str()) && str_lit.value.len() == 5 {
                        ...
                        if let Some(proxy) = self.proxies.get(str_lit.value.as_str()) {
                            match proxy {
                                Proxy::CallExpression { callee, arguments } => {
                                    ...
                                    let e_callee = if *callee < call_expr.arguments.len() {
                                        if let Some(mut expr) =
                                            call_expr.arguments[*callee].as_expression_mut()
                                        {
                                            Some(self.ast.move_expression(&mut expr))
                                        } else {
                                            None
                                        }
                                    } else {
                                ...
                                Proxy::BinaryExpression {
                                    ...
                                    let idx_left = arguments[0];
                                    let idx_right = arguments[1];

                                    let e_left = if idx_left < call_expr.arguments.len() {
                                        if let Some(mut expr) = call_expr.arguments[idx_left].as_expression_mut() {
                                            Some(self.ast.move_expression(&mut expr))
                                        } else {
                                            None
                                        }
                                    ...
                                    let e_right = if idx_right < call_expr.arguments.len() {
                                        if let Some(mut expr) = call_expr.arguments[idx_right].as_expression_mut() {
                                            Some(self.ast.move_expression(&mut expr))
                                        } else {
                                            None
                                        }
                                    ...
                                    if let (Some(left), Some(right)) = (e_left, e_right) {
